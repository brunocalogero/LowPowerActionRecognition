...
if __name__ == '__main__':
    ...
    data_set_path = 'C:/Users/Justin/Documents/LowPowerActionRecognition/CNN/datasets/Train/0/00002.bin'
    # X_train, y_train, X_test, y_test = load_NMNIST(data_set_path)
    td = read_dataset(data_set_path)
    print(td.width)
    print(td.height)
    # Getting the minima and maxima from ts
    t_min = np.min(td.data.ts) + 20000
    t_max = np.max(td.data.ts) - 20000
    ...
    for (i,item) in enumerate(p):
     ...
     #create a dataframe out of the 3 lists
    dict = {'x':x,'y':y,'p':p}
    ...
    #group by x and y, sum polarities for each x-y coordinate, and display x,y,p
    df2=df1.groupby(['x','y'])['x','y','p'].transform('sum')

    #drop duplicates and reset df index
    df3=df2.drop_duplicates(subset=['x', 'y','p']).reset_index(drop=True)
    print(df3)
